...
# labelling noise
# compare manual test annotations with synthetic OSM annotations

# plot_dir = "plots/manual_osm_%s/" % edition
# os.makedirs(plot_dir, exist_ok=True)
# results = score_predictions.calc_scores_dir(osm_masks_dir, test_masks_dir, plot_dir)
# with open(plot_dir + "labelling_noise_%s.txt" % edition, "w") as fw:
#     for line in results:
#         fw.write(str(line) + ",\n")
# print("manual vs osm %s: avg iou" % edition, sum([x["iou"] for x in results])/len(results))
# print("manual vs osm %s: avg ransac" % edition, sum([x["ransac"] for x in results])/len(results))
# print("manual vs osm %s: avg index rank" % edition, sum([x["index rank"] for x in results])/len(results))
...
# colour thresh usefulness
# compare colour thresholding masks with OSM annotations for all sheets
# plot_dir = "plots/colour_osm%s/" % edition
# os.makedirs(plot_dir, exist_ok=True)
# results = score_predictions.calc_scores_dir(colour_masks_dir, osm_masks_dir, plot_dir)
# with open(plot_dir + "colour_osm_%s.txt" % edition, "w") as fw:
#     for line in results:
#         fw.write(str(line) + ",\n")
# print("threshold vs osm: avg iou", sum([x["iou"] for x in results])/len(results))
# print("threshold vs osm: avg ransac", sum([x["ransac"] for x in results])/len(results))
