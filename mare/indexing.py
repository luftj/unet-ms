...
import mare.find_sheet as find_sheet
import mare.osm as osm
import config
...
def predict_annoy(descriptors):
    ...
    from mare.annoytest import get_sheet_for_id, sheets
...
def search_in_index(img_path, class_label_truth):
    ...
    return search_mask_in_index(water_mask, class_label_truth)

def search_mask_in_index(mask_img, class_label_truth):
    """
    Predict the identity of a single segmented map mask given as PIL Image or numpy array
    """
    # image size for intermediate processing
    processing_size = resize_by_width(mask_img.shape, config.index_img_width_query)
    mask_img_small = cv2.resize(mask_img, processing_size, interpolation=config.resizing_index_query)
    # extract features from query sheet
    kps, descriptors = extract_features(mask_img_small, first_n=config.index_n_descriptors_query)
